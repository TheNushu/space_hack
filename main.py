...
def rotational_dynamics(I_tensor, omega, torque, dt):
    ...
    omega_dot = np.dot(
        I_inv, torque - np.cross(omega, np.dot(I_tensor, omega)))
...
class OrbitSimulation:
    def __init__(self):
        self.G = ap_const.G.value
        self.M_earth = ap_const.M_earth.value
        self.R_earth = ap_const.R_earth.value
        self.dt = 10.0
        self.altitude = 800000
    ...
    def animate_orbit(self):
        ...
        def update(frame):
            ...
            trail.set_data(positions[start:frame, 0],
                           positions[start:frame, 1])
            ...
            field_line.set_data(
                time_hours[:frame], mag_field_magnitude[:frame] * 1e9)
...
# Assumptions:
# - homogenous mass distributions
# - alt = 800 km (perfect circ. orbit)
# - perfect cube - I same in the 3 middle axes
